"""
This file is adapted from "imagesegmentation.py" in the github repo "image-segmentation" by Julie Jiang.
A great portion of the functions are reused from the original file, while the others are changed or re-implemented to work in our problem framework.

The functions used to:
    - resize the input image
    - collect manually selected seeds and store them for later use on image sequences
...
# currently only support "ap" which is Ford-Fulkerson, later might include other graph-cut algorithms such as push-relabel.
...
"""
def imageSegmentation(imagename, folder, group, size=(30, 30), algo="ap", loadseed="yes"):
    ...
    seeds = np.loadtxt(seeds_directory, delimiter=',').astype(int) if loadseed == 'yes' else None
    ...
    if seededImage is not None:
        cv2.imwrite(seeded_image_dir + '/' + imagename.split('.')[0] + "_seeded.jpg", seededImage)

    begin = time.time()

    flows, cuts, path_count, average_path_len = graphCutAlgo[algo](graph, V, SOURCE, SINK)
    end = time.time()
